...
from utils.clean_util import CleanUtil
...
class AdvancedSpider(BaseSpider):
    ...
    def parse(self, response):
        ...
        for i in tweets.values():
            try:
                if CleanUtil.clean_lang_zh(i['lang']):  # 过滤中文
                    continue
                item = TweetItem()
                item['id'] = i['id']
    ...
    def parse_item(self, response):
        ...
        if replies[-1].get('entryId','').startswith('cursor'):
            ...
            item_['content'] = CleanUtil.clean_content(item_['content'])
            item_.pop('entities')
            item_.pop('reply_count')
            item_.pop('replies')
